...
Code, documentation, and test suites that once took days of engineering labor can now be
drafted in minutes.
...
requires control of the whole agent loop (assembly, budget, and retention); a context
window treated as a sliding attention viewport over a durable record, rather than as a
buffer the work must be compressed to fit; deterministic, extractive memory;
independent, tamper-resistant validation gates; mode-dependent context assembly; and
re-grounding at every step,
because per-step reliability compounds across the many steps of agentic work.
...
This paper is a position paper and a research agenda: a synthesis that assembles
converging work into a connected argument, takes falsifiable positions the field can
test, names a gap, the semantic and effectiveness levels, that current evidence does not
yet measure directly, and specifies the experiments that would test its distinctive
claims. We conclude with a set of practical
...
36\%, with per-model gains of 33--61\%). Section~12 takes up the relationship.} A name, though, is not yet a practice.
...
message; it was durability at the endpoints, sequencing, and a window that slides. And
consider the ratios involved: a terabyte over 1,500-byte frames is a mismatch of a
...
days, and the record of the work outgrows any window early (often within the first
...
The position is argued, not yet tested, and it is falsifiable: on long-horizon work, a
...
decision, that distant constraint, a scattered handful of code), and choosing it is a
...
Five commitments follow from this single move.
...
\subsection*{3.~Make validation independent and tamper-resistant}
...
\emph{deterministic and auditable}. The corpus itself is never condensed to fit the
window: the record accumulates, and the window carries only each step's projection of it
(Section~4). Where a projection must be condensed, prefer extraction (selecting verbatim
content with its provenance) over generative summary: a summary that hallucinates poisons
every future retrieval, while extraction can only omit, never invent.
...
distribution before generation begins. It narrows the selection problem for the same
reason: a task with checkable criteria names most of its own working set.
...
\subsection*{5.~Place independent validation gates before commits}
...
at a formal checkpoint. Where the generator is itself an LLM, the requirements belong in
the specification and the verification belongs out of the generator's hands: the gate
validates submitted evidence after the fact, with its implementation and held-out checks
independent; otherwise the model optimizes to pass the test rather than to do the work.
...
results. Assembly is viewport positioning, and the mode chooses the policy. One assembly
...
When a generation fails and the output does not match the intent, diagnose the
